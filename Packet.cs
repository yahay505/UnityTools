//https://github.com/tom-weiland/tcp-udp-networking/blob/tutorial-part4/GameClient/Assets/Scripts/Packet.cs
...
public class Packet : IDisposable
{
    ...
    {
        var x = Encoding.UTF8.GetBytes(_value);
    ...
    public uint ReadUInt(bool _moveReadPos = true)
    {
        ...
        {
            // If there are unread bytes
            var _value = BitConverter.ToUInt32(readableBuffer, readPos); // Convert the bytes to an int
            if (_moveReadPos)
            {
                // If _moveReadPos is true
                readPos += 4; // Increase readPos by 4
